...
Application and test files are preserved from the source snapshot. Only the two
environment configurations are generalized; documentation, licensing, ignore
rules, and test-install instructions are added. Personal work reports, old Git
history, local virtual environments, credentials, and trained models are excluded.
...
4. Run the personal accelerator's initialization pipeline with
   `classical`, `aml-cli-v2`, `bicep`, and the repaired taxi template enabled.
5. Configure the generated project before running its deployment pipelines.

Both personal GitHub repositories are private initially. Readers need GitHub
access and must authenticate the Azure Repos import themselves. Never put a PAT
...
- Check regional Azure ML compute quota, image access, and managed-online SKU
  support. The copied online setting is one `Standard_D2as_v4` instance, not a
...
- **Important infrastructure prerequisite:** the copied compute-cluster Bicep
  ...
  is not transported by copying a repository. The infrastructure cluster defaults
  to `Standard_DS3_v2`; inspect its quota separately from online deployment.
...
The copied suites contain 34 Node/Bash/CLI cases and 18 Python cases. Python tests
